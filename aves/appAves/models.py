...
class ave(models.Model):
    ...
    synonim = models.CharField(max_length=0)
    ...
    id_endemismo = models.ForeignKey(endemismo)
    id_migracion = models.ForeignKey(migracion)
    id_autor = models.ForeignKey(autor)
    id_bibliografia = models.ForeignKey(bibliografia)
	def __unicode__(self):
        return "%s - %s" % (self.id, self.codigo, self.especie, self.synonim, self.nombre, self.morfometres, self.ecology, self.ecology, self.behaviour,
                           self.yearPublished, self.yearCollection, self.date, self.call, self.observacion, self.id_familia, self.id_endemismo, self.id_migracion,
                           self.id_autor, self.id_bibliografia)
